...
type SchemaUserTechProjectLink struct {
...
type SchemaUserTechProject struct {
	...
	Links       []SchemaUserTechProjectLink `json:"links" validate:"dive"`
	Attachments Attachments                 `json:"attachments" validate:"required,min=1,max=20,dive"`
}

func (s SchemaUserTechProject) Validate() error {
	validate := validator.New()
	return validate.Struct(s)
}

type SchemaUserTechProjectMetadata struct {
...
func (s *SchemaUserTechProjectMetadata) Validate() error {
	validate := validator.New()
	return validate.Struct(s)
}
