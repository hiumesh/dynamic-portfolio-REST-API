...
type ConfigFunc func(*config.GlobalConfiguration)
...
func (rl *Reloader) reload() (*config.GlobalConfiguration, error) {
	if err := config.LoadDirectory(rl.watchDir); err != nil {
		return nil, err
	}

	cfg, err := config.LoadGlobalFromEnv()
	if err != nil {
		return nil, err
	}
	return cfg, nil
